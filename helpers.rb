...
helpers do
  ...
  def lgas_stats(lgas)
    ...
    lgas_max_mins = { total: { energy: nil, customers: nil, population: nil },
                      residential: { total: { energy: nil, customers: nil }, normal: { energy: nil, customers: nil }, controlled_load: { energy: nil, customers: nil } }, business: { total: { energy: nil, customers: nil }, small: { energy: nil, customers: nil }, large: { energy: nil, customers: nil } } }
    # Totals
    ...
    lgas_max_mins[:residential][:total][:per_customer] =
      statistical_data(@lgas.collect(&:total_residential_energy_per_customer))
    lgas_max_mins[:residential][:total][:per_resident] =
      statistical_data(@lgas.collect(&:total_residential_energy_per_resident))
    ...
    lgas_max_mins[:residential][:normal][:per_customer] =
      statistical_data(@lgas.collect(&:residential_energy_per_customer))
    lgas_max_mins[:residential][:normal][:per_resident] =
      statistical_data(@lgas.collect(&:residential_energy_per_resident))

    lgas_max_mins[:residential][:controlled_load][:energy] =
      statistical_data(@lgas.collect(&:residential_controlled_load_energy))
    lgas_max_mins[:residential][:controlled_load][:customers] =
      statistical_data(@lgas.collect(&:residential_controlled_load_customers))
    lgas_max_mins[:residential][:controlled_load][:per_customer] =
      statistical_data(@lgas.collect(&:residential_controlled_load_energy_per_customer))
    lgas_max_mins[:residential][:controlled_load][:per_resident] =
      statistical_data(@lgas.collect(&:residential_controlled_load_energy_per_resident))
    ...
    lgas_max_mins[:business][:total][:per_customer] =
      statistical_data(@lgas.collect(&:total_business_energy_per_customer))
    ...
    lgas_max_mins[:business][:small][:per_customer] =
      statistical_data(@lgas.collect(&:small_business_energy_per_customer))
    ...
    lgas_max_mins[:business][:large][:per_customer] =
      statistical_data(@lgas.collect(&:large_business_energy_per_customer))
  ...
  def statistical_data(arg)
    return nil if arg.nil? || arg.none?
    ...
    variance = variance(arg)
    {
      ...
      max: arg.max,
      min: arg.min,
      average: arg.sum.to_f / arg.length,
      variance: variance,
  ...
  def color_gradient(value, opts)
    ...
    # average = "39a9ff"
    min = '49cd6e'
    value.to_f! if value.instance_of?('Fixnum')
    # puts value.class
    opts[:max]
    ...
    if value <= opts[:min]
      min
    elsif value == opts[:average]
      average
    elsif value >= opts[:max]
      max
    elsif value > opts[:min] && value < opts[:average]
      red = (min[0..1].hex.to_i + (percent_max_min_lower * (average[0..1].hex.to_i - min[0..1].hex.to_i))).to_i.to_s(16).rjust(
        2, '0'
      )
      green = (min[2..3].hex.to_i + (percent_max_min_lower * (average[2..3].hex.to_i - min[2..3].hex.to_i))).to_i.to_s(16).rjust(
        2, '0'
      )
      blue = (min[4..5].hex.to_i + (percent_max_min_lower * (average[4..5].hex.to_i - min[4..5].hex.to_i))).to_i.to_s(16).rjust(
        2, '0'
      )
      [red, green, blue].join
    elsif value > opts[:average] && value < opts[:max]
      red = (average[0..1].hex.to_i + (percent_max_min_upper * (max[0..1].hex.to_i - average[0..1].hex.to_i))).to_i.to_s(16).rjust(
        2, '0'
      )
      green = (average[2..3].hex.to_i + (percent_max_min_upper * (max[2..3].hex.to_i - average[2..3].hex.to_i))).to_i.to_s(16).rjust(
        2, '0'
      )
      blue = (average[4..5].hex.to_i + (percent_max_min_upper * (max[4..5].hex.to_i - average[4..5].hex.to_i))).to_i.to_s(16).rjust(
        2, '0'
      )
      [red, green, blue].join
    end
